...
def generate_corresp(tracks, num, fps1, fps2, offset1, offset2):
    pot_1 = 0
    #pot_2 = 0
    ...
    for i in range(len(tracks)/2):
        ...
        for index in range(tracks[i*2].shape[0]):
            time1[index] = frame2time(tracks[i*2][index][0], fps1, offset1)

        add_num2 = num[i*2+1]

        time2 = np.zeros(add_num2)
        for ind in range(tracks[i*2+1].shape[0]):
            time2[ind] = frame2time(tracks[i*2+1][ind][0], fps2, offset2)
