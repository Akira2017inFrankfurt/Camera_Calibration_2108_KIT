...
//load file 1
        std::vector<std::string> filenames_1; 
        std::vector<std::string> imagenames;

        std::vector<cv::Mat> frames;
        cv::glob(FLAGS_i1, filenames_1);
        for (size_t i = 0; i < filenames_1.size(); ++i)
        {
            std::size_t found = filenames_1[i].find_last_of("/");
            imagenames.push_back( filenames_1[i].substr( found +1 ));
            cv::Mat myImage = cv::imread(filenames_1[i], cv::IMREAD_COLOR);
            frames.push_back(myImage);
        }
        const bool isempty = frames.empty();
        const size_t width = frames[0].size().width;
        const size_t height = frames[0].size().height;
        int frames_size_1 = frames.size();

//load file 2
        std::vector<std::string> filenames_2;
        cv::glob(FLAGS_i, filenames_2);
        for (size_t i = 0; i < filenames_2.size(); ++i)
        {
            std::size_t found = filenames_2[i].find_last_of("/");
            imagenames.push_back( filenames_2[i].substr( found +1 ));
            cv::Mat myImage = cv::imread(filenames_2[i], cv::IMREAD_COLOR);
            frames.push_back(myImage); 
        }       
        int frames_size_2 = frames.size() - frames_size_1;
...
//load file 2
        ...
        */
        ...
        std::vector<PersonInfo> outputInformation;
        outputInformation.reserve(frames_size);    	//allocate merory
        ...
        do {
            ...
            for (auto && result : personDetection.results) {
                ...
                if (result.label == 1) {  // person
                    ...
                    std::cout << "REID: "<< PersonID << std::endl;
                    std::cout << "BoundingBox: "<< result.location << std::endl;     //import change
                    PersonInfo p;
                    p.NumFrame = num;
                    p.Reid = PersonID;
                    p.BoundingBox = result.location;
                	outputInformation.push_back(p);
                }
            }
            ...
            // don't know how to imwrite
            if (num < frames_size_1)
            {   
                std::string image_path = FLAGS_o1 + "/" + imagenames[num];
                // std::cout<<"the Path:"<< image_path <<std::endl;                
                cv::imwrite (image_path, frame);
            }
            else
            {   
                std::string image_path = FLAGS_o2 + "/" +imagenames[num];
                cv::imwrite (image_path, frame);   
            }
            //
            num++;
        } while (num < frames_size);

        std::ofstream output1(FLAGS_o1 + "/" + "data1.txt");
        std::ofstream output2(FLAGS_o2 + "/" + "data2.txt");
        for(auto resulte : outputInformation){
        	if(resulte.NumFrame < frames_size_1 && output1.is_open()){
        		output1 << imagenames[resulte.NumFrame] << " ";
        		output1 << resulte.Reid <<" ";
        		output1 << resulte.BoundingBox.x <<" "<<resulte.BoundingBox.y<<" "
        				<<resulte.BoundingBox.width <<" " <<resulte.BoundingBox.height << std::endl;
        	}
        	else{
        		if(output2.is_open()){
        			output2 << imagenames[resulte.NumFrame] << " ";
        			output2 << resulte.Reid <<" ";
        			output2 << resulte.BoundingBox.x <<" "<<resulte.BoundingBox.y<<" "
        					<<resulte.BoundingBox.width <<" " <<resulte.BoundingBox.height  << std::endl;
        		}
